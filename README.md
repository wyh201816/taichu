...
写到宗门大会，你让太初回查前二十章的伏笔、对照几版设定，再按审查意见重写。几十轮讨论之后，下一步可能只需续写两千字，正文、检索结果和修改记录却仍在不断累积。只留下最近几轮，又可能漏掉最早的约束：**“这一卷不能突破，掌门身份不能提前揭露。”**

太初先为大结果生成可回读的预览，清理陈旧结果，持续整理目标、进度与约束；窗口吃紧时，再把旧对话折叠为摘要，必要时重整整个活动上下文。为下一步创作腾出空间，同时保留完整原始记录供回查。
...
太初处于 开发者预览 阶段，正在快速迭代。**未来可能出现破坏兼容性的变更。**
...
- Skill的加载方式留给用户，后续用最简单的引导帮助用户建立自己想要的技能。本项目本身大量用到这种渐进性披露的能力，但不一定依赖于 `skill.md` 这个文件；把自定义技能的能力留给用户也不错。
...
- 优化长链路的时间、Token消耗与调用成本。
- 基于真实数据分析、作者反馈和使用经验来迭代Prompt规则定义，并探索受评测约束的提示词自进化。
- 探索用户、协作和小说画像的长期记忆，调研 LLM-Wiki、LoCoMo、Memobase 等相关思路。希望系统能综合跨越多个、甚至很久以前的会话信息，提供有预见性的主动帮助，从看似无关的记忆中发现深层联系。比如用户提到最近进展不佳时，主动从多轮对话提取出用户可能遇到的困难并提供帮助。用户没有说让它做什么，它自己就知道做——这是Harness为智能体赋予的另一层智能。
